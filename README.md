# Algorithms
# CNN + Hyperparameter tuning (MNIST)
Convolutional Neural Network optimized with randomized hyperparameter search for accurate digit classification on the MNIST dataset.
  -------------------------------------------------------------------------------------------------------------------------
Best Hyperparameters: {'model__pool_size': (2, 2), 'model__learning_rate': 0.001, 'model__kernel_size': (5, 5), 'model__filters': 32, 'epochs': 5, 'batch_size': 64}

Test accuracy: 0.9911
# LSTM + Hyperparameter Tuning (Airline)
Long Short-Term Memory network for forecasting monthly airline passenger numbers, with hyperparameter optimization and sequence-based input preparation.
  --------------------------------------------------------------------------------------------------------------------------
<img width="1000" height="600" alt="l_c_fig1" src="https://github.com/user-attachments/assets/d553f7b3-31ff-490e-8953-f77ccec8ae2a" />
<img width="1000" height="600" alt="l_c_fig2" src="https://github.com/user-attachments/assets/e3c842af-293b-4339-8073-c5593b3c03eb" />
Best Hyperparameters:
{'activation': 'tanh', 'batch_size': 16, 'dropout_rate': np.float64(0.11393512381599932), 'epochs': 100, 'num_layers': 2, 'optimizer': 'adam', 'units': 128}

Test Loss: 0.0061
# GAN (MNIST-dataset)
Generative Adversarial Network trained to generate realistic handwritten digit images, demonstrating deep generative modeling capabilities.
 ------------------------------------------------------------------------------------------------------------------------- 
The real dataset :

<img width="630" height="500" alt="gan_fig1" src="https://github.com/user-attachments/assets/d7ebf7d2-742c-477f-bfe5-6a3fc65dd30e" />

The synthetic data after just 20 epoch :

<img width="620" height="550" alt="gan_fig2(200epoch)" src="https://github.com/user-attachments/assets/d0c874bd-ddbf-4393-95a7-e7f75f663d7f" />

# GNN (GCN) for Classification (PubMed -dataset) 
Graph Convolutional Network applied for node classification in the PubMed citation network, including validation/test evaluation and graph visualization.
 ------------------------------------------------------------------------------------------------------------------------- 
Final Test Accuracy: 0.7690
<img width="1000" height="800" alt="gnn_pubMed" src="https://github.com/user-attachments/assets/e6087e60-9f3c-46d6-9797-dae7bb9b89fd" />
# Node Classification with GCN on Karate Club Graph
GCN-based node classification on the Karate Club graph with visualization of predicted communities and structure-aware learning.
 -------------------------------------------------------------------------------------------------------------------------- 
Before classification :

<img width="800" height="600" alt="gnn1_fig1" src="https://github.com/user-attachments/assets/95af8750-991f-4425-b3d9-cd1f3d87f1b3" />

The classified graph: 

<img width="800" height="600" alt="Figure_1" src="https://github.com/user-attachments/assets/29cebb15-e609-4976-b4ae-b7518bc08e5e" />
